...
class atlas_labeler():
	...
	def __init__(self):
		...
		self.atlas_list = ['allvoxel/rbrodmann.nii','allvoxel/atlas_labels_combined.img','allvoxel/ROI_MNI_V4.nii']
		self.labels_list = ['allvoxel/rbrodmann.txt','allvoxel/names_atlas_labels_combined.txt','allvoxel/ROI_MNI_V4_names.txt']
		self.json_files = ['data/rbrodmann.json','data/atlas_labels_combined.json','data/ROI_MNI_V4.json']
	...
	def load_atlas_roi_data(self,atlas_name):
		...
		print ('Loading data from ' + return_atlas_file(atlas_name))
	...
	def get_atlas_labels(self,atlas_name):
		"""eturn the labels for each brain area in atlas. options = brodmann, combined, aal"""
		if atlas_name == 'brodmann':
